...
CBackSmoke::CBackSmoke(LPDIRECT3DDEVICE9 pGraphicDev)
	: CGameObject(pGraphicDev)
{
	m_pResourceMgr = Engine::Get_ResourceMgr();
}

CBackSmoke::~CBackSmoke(void)
{
	Release();
}

HRESULT CBackSmoke::Initialize(void)
{
	...
	m_pTexName = L"Texture_BackSmoke";
...
void CBackSmoke::Update(void)
{
	//SetTransform();
}

void CBackSmoke::LateUpdate(void)
{
}

void CBackSmoke::Render(void)
{
	m_pGraphicDev->SetRenderState(D3DRS_LIGHTING, TRUE);
	m_pGraphicDev->SetMaterial(&m_tMtrl);		//ÀçÁú¡Ú

	m_pInfoCom->m_vScale = D3DXVECTOR3(0.6f, 0.6f, 1.f);

	m_pInfoCom->m_vPos.x = -0.8f;
	m_pInfoCom->m_vPos.y = 1.f;
	m_pInfoCom->m_vPos.z = 2.f;
	SetTransform();
	...
	m_pInfoCom->m_vScale = D3DXVECTOR3(0.7f, 0.7f, 1.f);
	...
	m_pInfoCom->m_vPos.x = 1.2f;
	m_pInfoCom->m_vPos.y = 0.75f;
	m_pInfoCom->m_vPos.z = 2.f;
	SetTransform();
	m_pResourceMgr->SendToEngine(Engine::RESOURCE_STATIC, m_pBufferName, m_pConvertVtx);
	...
	SetTransform();
	m_pResourceMgr->SendToEngine(Engine::RESOURCE_STATIC, m_pBufferName, m_pConvertVtx);
	m_pTextureCom->SetTexture(4);
	m_pBufferCom->Render();

	m_pInfoCom->m_vScale = D3DXVECTOR3(0.6f, 0.6f, 1.f);
	m_pInfoCom->m_vPos.x = 1.55f;
	m_pInfoCom->m_vPos.y = -1.05f;
	m_pInfoCom->m_vPos.z = 1.4f;
	SetTransform();
	m_pResourceMgr->SendToEngine(Engine::RESOURCE_STATIC, m_pBufferName, m_pConvertVtx);
...
void CBackSmoke::SetTransform(void)
{
...
HRESULT CBackSmoke::AddComponent(void)
{
...
CBackSmoke * CBackSmoke::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CBackSmoke* pInstance = new CBackSmoke(pGraphicDev);
