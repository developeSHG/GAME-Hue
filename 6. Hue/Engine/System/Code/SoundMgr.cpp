...
{
	for (int i = 0; i < Engine::CHANNELID::MAXCHANNEL; ++i)
	{
...
void CSoundMgr::MyPlaySound(TCHAR * pSoundKey, Engine::CHANNELID eID)
{
...
void CSoundMgr::PlayBGM(TCHAR * pSoundKey, bool bReduceVolume/* = false*/)
{
	if (m_mapSound.empty())
		return;
	...
	if (bReduceVolume)
		FMOD_Channel_SetVolume(m_pChannelArr[Engine::CHANNELID::BGM], 0.1f);
	else

		FMOD_Channel_SetVolume(m_pChannelArr[Engine::CHANNELID::BGM], 0.5f);
}

void CSoundMgr::StopSound(Engine::CHANNELID eID)
{
