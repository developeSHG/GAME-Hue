...
HRESULT CCubeLaser::Initialize(void)
{
	...
	m_pInfoCom->m_vScale.x = 0.5f;
	m_pInfoCom->m_vScale.y = 0.5f;
...
void CCubeLaser::SetTransform(void)
{
	...
	if (Engine::CUBE_MOVING_DIR::CUBE_UP == m_tCubeInfo.tPlatformInfo.eDir)
		D3DXMatrixRotationZ(&matRot, D3DXToRadian(-90.f));
	else if (Engine::CUBE_MOVING_DIR::CUBE_LEFT == m_tCubeInfo.tPlatformInfo.eDir)
		D3DXMatrixRotationZ(&matRot, D3DXToRadian(0.f));
